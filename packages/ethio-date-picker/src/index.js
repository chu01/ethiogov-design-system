...
const selectOrMatches = require("../../ewds-core/src/js/utils/select-or-matches");
const { prefix: PREFIX } = require("../../ewds-core/src/js/config");
const { CLICK } = require("../../ewds-core/src/js/events");
const activeElement = require("../../ewds-core/src/js/utils/active-element");
const isIosDevice = require("../../ewds-core/src/js/utils/is-ios-device");
const Sanitizer = require("../../ewds-core/src/js/utils/sanitizer");
...
const processFocethiobleSelectors = (...selectors) =>
  selectors.map((query) => query + NOT_DISABLED_SELECTOR).join(", ");

const DATE_PICKER_FOCethioBLE = processFocethiobleSelectors(
...
const MONTH_PICKER_FOCethioBLE = processFocethiobleSelectors(
  CALENDAR_MONTH_FOCUSED,
);

const YEAR_PICKER_FOCethioBLE = processFocethiobleSelectors(
...
const enhanceDatePicker = (el) => {
  ...
  calendarWrapper.insertAdjacentHTML(
    ...
    <div class="ethio-sr-only ${DATE_PICKER_STATUS_CLASS}" role="status" aria-live="polite"></div>`,
  );
...
const tabHandler = (focethioble) => {
  const getFocethiobleContext = (el) => {
    const { calendarEl } = getDatePickerContext(el);
    const focethiobleElements = select(focethioble, calendarEl);

    const firstTabIndex = 0;
    const lastTabIndex = focethiobleElements.length - 1;
    const firstTabStop = focethiobleElements[firstTabIndex];
    const lastTabStop = focethiobleElements[lastTabIndex];
    const focusIndex = focethiobleElements.indexOf(activeElement());
  ...
  return {
    tabAhead(event) {
      const { firstTabStop, isLastTab, isNotFound } = getFocethiobleContext(
        event.target,
      );
    ...
    tabBack(event) {
      const { lastTabStop, isFirstTab, isNotFound } = getFocethiobleContext(
        event.target,
      );
...
const datePickerTabEventHandler = tabHandler(DATE_PICKER_FOCethioBLE);
const monthPickerTabEventHandler = tabHandler(MONTH_PICKER_FOCethioBLE);
const yearPickerTabEventHandler = tabHandler(YEAR_PICKER_FOCethioBLE);
