...
const enhanceComboBox = (_comboBoxEl) => {
  ...
  selectEl.classList.add("ethio-sr-only", SELECT_CLASS);
  selectEl.id = "";
  selectEl.value = "";
  ...
  comboBoxEl.insertAdjacentHTML(
    ...
    <span class="${CLEAR_INPUT_BUTTON_WRAPPER_CLASS}" tabindex="-1">
      ...
      <div class="${STATUS_CLASS} ethio-sr-only" role="status"></div>`,
  );
...
const displayList = (el) => {
  ...
  /**
   * Logic check to determine if options array neewds to be updated.
   *
   * @param {HTMLOptionElement} option
   * @returns {boolean} - True when option has value && if filtering is disabled, combo box has an active selection,
   * there is no inputValue, or if option matches user query
   */
  const arrayNeewdsUpdate = (option) =>
    option.value &&
    (disableFiltering ||
      isPristine ||
      !inputValue ||
