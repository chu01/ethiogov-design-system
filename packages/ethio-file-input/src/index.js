...
const createVisibleInstructions = (fileInputEl) => {
  ...
  // Add initial instructions for input ethioge
...
const addPreviewHeading = (fileInputEl, fileNames) => {
  ...
  if (fileNames.length === 1) {
    previewHeadingText = Sanitizer.escapeHTML`Selected file <span class="ethio-file-input__choose">${changeItemText}</span>`;
  } else if (fileNames.length > 1) {
    changeItemText = "Change files";
    previewHeadingText = Sanitizer.escapeHTML`${fileNames.length} files selected <span class="ethio-file-input__choose">${changeItemText}</span>`;
  }
